...
class PostListView(PostMixin, ListView):
    ...
    def get_queryset(self):
        return self.get_post_queryset().annotate(comment_count=models.Count('comments'))
...
def category_posts_view(request, category_slug):
    category = get_object_or_404(Category.objects.filter(slug=category_slug,
                                                         is_published=True))
    queryset = PostMixin().get_post_queryset().filter(category=category).annotate(
...
class ProfileDetailView(PostMixin, ListView):
    ...
    def get_queryset(self):
        if self.kwargs["username"] == self.request.user.username:
            ...
            return PostMixin().get_post_queryset().filter(author__username=self.kwargs["username"]) \
                .annotate(comment_count=Count("comments"))
